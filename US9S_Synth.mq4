//+------------------------------------------------------------------+
//|  US9S_Synth.mq4 — Synthetic US9S (-3x inverse S&P 500)           |
...
#property indicator_label1  "US9S (-3x)"
#property indicator_type1   DRAW_LINE
#property indicator_color1  clrRed
#property indicator_style1  STYLE_SOLID
#property indicator_width1  2

input double Seed_US9S = 25.0;  // Current REAL market price of US9S
...
int OnInit()
{
   SetIndexBuffer(0, g_US9S);
   SetIndexStyle(0, DRAW_LINE, STYLE_SOLID, 2, clrRed);
   IndicatorShortName("US9S -3x [seed="
      + DoubleToString(Seed_US9S, 2) + "]");
   ArraySetAsSeries(g_US9S, true);
   g_initialized = false;
   ...
   const long &volume[], const int &spread[])
{
   ...
   {
      g_US9S[0] = Seed_US9S;
      double be = Seed_US9S, bc = iClose(Symbol(), PERIOD_M1, 0);
      for(int i = 1; i < n; i++)
      {
         double ci = iClose(Symbol(), PERIOD_M1, i);
         if(ci <= 0.0) { g_US9S[i] = g_US9S[i-1]; continue; }
         if(IsDayBoundary(i-1))
            { be = g_US9S[i-1]; bc = iClose(Symbol(),PERIOD_M1,i-1); }
         if(bc <= 0.0) { g_US9S[i] = g_US9S[i-1]; continue; }
         g_US9S[i] = be * MathPow(ci / bc, -3.0); // L=-3
      }
   ...
   for(int k = 1; k < 1500; k++)
      if(IsDayBoundary(k))
      {
         ...
         if(cb > 0.0 && cn > 0.0)
            g_US9S[0] = g_US9S[k] * MathPow(cn/cb, -3.0);
         break;
      }
   return(rates_total);
}
